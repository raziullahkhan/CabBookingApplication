...
public class TripBookingControllers {
	...
	@PostMapping("/add")
	public TripBooking addTrip(@RequestBody TripBooking tripBooking) throws CustomerNotFoundException {
		
		try {
			Customer customer=customerService.viewCustomer(tripBooking.getCustomerId());
		}
		catch(Exception e) {
			throw new CustomerNotFoundException("Customer not found with id "+tripBooking.getCustomerId());
		}
		return tripService.insertTripBooking(tripBooking);
	}
	...
	public TripBooking deleteTripBooking(@PathVariable int tripBookingId) throws TripBookingNotFoundException{
		TripBooking trip=null;
		try {
